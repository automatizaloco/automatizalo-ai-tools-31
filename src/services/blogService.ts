...
export const createBlogPost = async (postData: Omit<BlogPost, "id">): Promise<BlogPost> => {
  ...
  const translations = postData.translations || await generateTranslations(
    postData.title,
    postData.excerpt,
    postData.content
  );
